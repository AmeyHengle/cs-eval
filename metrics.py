...
from sklearn.metrics import (accuracy_score, f1_score, mean_absolute_error,
                             mean_squared_error, ndcg_score, jaccard_score)
...
class Metrics:
    ...
    def compute_novelty_score(self, predictions, references):
        ...
        for prediction in tqdm(predictions, desc="Computing novelty score"):
            ...
            jaccard_similarity_scores = [jaccard_similarity(prediction, reference) for reference in references]
    ...
    def compute_diversity_score(self, predictions):
        ...
        for prediction in tqdm(predictions, desc="Computing diversity score"):
            # Calculate the Jaccard similarity between the summary and the text
            jaccard_similarity_scores = [jaccard_similarity(prediction, x) for x in predictions if x != prediction]
            # Return the novelty as 1 minus the maximum similarity
    ...
    def compute_category_accuracy(
        ...
        model_path="/home/ameyh/depository/Counter-Speech-Generation/checkpoints/best_model-gpt-data-ca-fix2-roberta-large",
        batch_size=16,
    ):
    ...
    def compute_pc_cd_aq_scores(self, hatespeech, counterspeech, batch_size=100):
        ...
        for i in tqdm(
            ...
            input = [{'sentence': cs, 'topic': hs} for cs, hs in zip(batch_inputs_hatespeech, batch_inputs_counterspeech)]
    ...
    def get_classification_metrics(self, predicted_rankings, true_rankings):
        predicted_rankings = [ast.literal_eval(x) for x in predicted_rankings if isinstance(x, str)]
        true_rankings = [ast.literal_eval(x) for x in true_rankings if isinstance(x, str)]
        ...
        average_precision, prec_at_k = self.compute_precision_score(predicted_rankings, true_rankings)
    ...
    def get_generation_metrics(self, df):
        ...
        hatespeech = df["hatespeech"].apply(lambda x: clean_text(x).lower()).values.tolist()
        counterspeech = (
        ...
        # hs_ = df["hatespeech"].sample(100, random_state=42).apply(lambda x: clean_text(x).lower()).values.tolist()
        # predicted_cs_ = (
        #     df["prediction_m2"].sample(100, random_state=42).apply(lambda x: clean_text(x).lower()).values.tolist()
        # )
        ...
        # print("-" * 50)
        # print(f"Calculating PC Score")
        # pc_score_entailment, pc_score_contradiction = self.compute_pc_score(s1=hatespeech, s2=predictions_m2)

        # print("-" * 50)
        # print(f"Calculating CD Score")
        # cd_score = self.compute_cd_score(hatespeech=hatespeech, counterspeech=predictions_m2)

        pc_score, cd_score, aq_score = self.compute_pc_cd_aq_scores(hatespeech=hatespeech, counterspeech=predictions_m2)

        # print("-" * 50)
        # print(f"Calculating PD Score")
        # pd_score, pd_score_norm, pd_score_norm_toxicity = self.compute_pd_score(
        #     hatespeech, predictions_m2, cd_score, pc_score, toxicity_score
        # )

        # print("-" * 50)
        # print(f"Calculating Novelty Score")
        # novelty_score = self.compute_novelty_score(predictions=predictions_m2, references=counterspeech)

        # print("-" * 50)
        # print(f"Calculating Diversity Score")
        # diversity_score = self.compute_diversity_score(predictions=predictions_m2)
        ...
        return {
            ...
            # "pc_score_entailment": pc_score_entailment,
            # "pc_score_contradiction": pc_score_contradiction,
            "pc_score": pc_score,
            "cd_score": cd_score,
            "arg_quality_score": aq_score,
            # "pd_score": pd_score,
            # "pd_score_norm": pd_score_norm,
            # "pd_score_norm_toxicity": pd_score_norm_toxicity,
...
if __name__ == "__main__":
    ...
    hatespeech=hatespeech, counterspeech=counterspeech, batch_size=400
    )
    ...
    input_df.to_csv(
        '/home/ameyh/counterspeech-EVAL/preds/results_combined.csv',
        index=False
    )